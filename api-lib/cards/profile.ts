...
function stat(x: number, label: string, value: string, accent: string) {
  return `<g transform="translate(${x},0)"><circle cx="14" cy="18" r="14" fill="${accent}26"/><text x="14" y="23" text-anchor="middle" font-family="${FONT}" font-size="14" font-weight="700" fill="${accent}">·</text><text x="38" y="17" font-family="${FONT}" font-size="10" letter-spacing="1.2" fill="${MUTED}">${esc(label.toUpperCase())}</text><text x="38" y="38" font-family="${FONT}" font-size="19" font-weight="700" fill="${TEXT}">${esc(value)}</text></g>`;
}
...
function render(data: LiveData) {
  ...
  ${panel(48,346,530,112,heading('GitHub activity', 'Live public profile data') + stat(24,'Repos',String(profile.repos),BLUE) + stat(160,'Followers',String(profile.followers),VIOLET) + stat(296,'Following',String(profile.following), '#22c55e'))}
